...
export default function Warga() {
  ...
  useEffect(() => {
    if (latlng && appSettings) {
      // OSRM format: lng,lat;lng,lat
      fetch(`https://router.project-osrm.org/route/v1/driving/${latlng.lng},${latlng.lat};${appSettings.tpaLng},${appSettings.tpaLat}?overview=false`)
        .then(res => res.json())
        .then(data => {
  ...
  const bayarManual = async () => {
    ...
    if (!buktiFile) return alert("Silakan upload bukti pembayaran terlebih dahulu!");
    setPayLoading(true);
    try {
      ...
      const fileName = `bukti-bayar-${wargaData.id}-${Date.now()}.${fileExt}`;
      ...
      const buktiUrl = supabase.storage.from('uploads').getPublicUrl(fileName).data.publicUrl;
      ...
      setBuktiFile(null);
      alert("Pembayaran manual berhasil diajukan dengan bukti transfer! Menunggu verifikasi admin.");
      refreshHistory(wargaData.id);
    } catch (err) {
  ...
  let totalBiaya = appSettings.baseFee;

  if (latlng && !osrmDistance) {
    // Fallback ke garis lurus jika OSRM loading/gagal
    jarakKeTpa = getDistanceFromLatLonInKm(latlng.lat, latlng.lng, appSettings.tpaLat, appSettings.tpaLng);
  }

  if (jarakKeTpa > 0) {
    totalBiaya = appSettings.baseFee + Math.round(jarakKeTpa * appSettings.feePerKm);
  }
                ...
                <div className="map-container-wrapper" style={{ marginTop: 0 }}>
                  ...
                  <div className="grid-2-col" style={{ marginBottom: "20px" }}>
                    {[
                      ...
                      { label: "Biaya Dasar", value: `Rp ${appSettings.baseFee.toLocaleString("id-ID")}` },
                      { label: `Ongkos Jarak ke TPA (${jarakKeTpa.toFixed(1)} km)`, value: `Rp ${(Math.round(jarakKeTpa * appSettings.feePerKm)).toLocaleString("id-ID")}` },
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px", alignItems: "center", backgroundColor: "#f8fafc", padding: "20px", borderRadius: "12px", border: "1px dashed #cbd5e1" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px", width: "100%", maxWidth: "300px" }}>
                      ...
                      <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "12px", marginTop: "4px" }}>
                        <p style={{ margin: "0 0 8px 0", fontSize: "12px", color: "var(--color-text-muted)", textAlign: "center" }}>Atau bayar manual dengan upload bukti transfer:</p>
                        <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", padding: "12px", border: "2px dashed #cbd5e1", borderRadius: "8px", cursor: "pointer", backgroundColor: buktiFile ? "#ecfdf5" : "#fff", transition: "all 0.2s" }}>
                          <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke={buktiFile ? "#059669" : "#94a3b8"} strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                          <span style={{ fontSize: "13px", fontWeight: 600, color: buktiFile ? "#059669" : "#64748b" }}>
                            {buktiFile ? `✓ ${buktiFile.name}` : "Pilih Bukti Transfer"}
                          </span>
                          <input type="file" accept="image/*" style={{ display: "none" }} onChange={(e) => setBuktiFile(e.target.files[0] || null)} />
                        </label>
                        {buktiFile && (
                          <div style={{ marginTop: "6px", textAlign: "center" }}>
                            <img src={URL.createObjectURL(buktiFile)} alt="Preview" style={{ maxWidth: "100%", maxHeight: "120px", borderRadius: "6px", border: "1px solid #e2e8f0" }} />
                          </div>
                        )}
                      </div>

                      <button onClick={bayarManual} disabled={payLoading || !buktiFile} style={{ background: buktiFile ? "#059669" : "#e2e8f0", color: buktiFile ? "#fff" : "#475569", padding: "10px 16px", borderRadius: "8px", fontSize: "14px", fontWeight: "bold", border: "none", cursor: buktiFile ? "pointer" : "not-allowed", width: "100%", transition: "all 0.2s" }}>
                        {payLoading ? "Memproses..." : buktiFile ? "📤 Upload & Bayar Manual" : "Upload Bukti Dulu untuk Bayar Manual"}
                      </button>
                    </div>
                  </div>
                </div>
