...
export default function Courier() {
  ...
  const updateTugas = async (id, status, file = null) => {
    ...
    if (status === "selesai") {
      ...
      // Catat pendapatan
      const { error: pendapatanError } = await supabase.from("transaksi_courier").insert({
        courier_id: myId,
        jumlah: 15000,
        keterangan: "Fee penjemputan #" + id
      });
      if (pendapatanError) {
        console.error("Gagal catat pendapatan:", pendapatanError);
        alert("⚠️ Tugas selesai, tapi gagal mencatat pendapatan: " + pendapatanError.message + "\n\nKemungkinan penyebab: RLS policy belum diatur untuk tabel transaksi_courier.");
      }
    }
      ...
      {myId && (
        <>
          <button onClick={() => setChatOpen(!chatOpen)} style={{ position: "fixed", bottom: "20px", right: "20px", width: "56px", height: "56px", borderRadius: "28px", backgroundColor: "#3b82f6", color: "#fff", border: "none", boxShadow: "0 4px 12px rgba(59, 130, 246, 0.4)", cursor: "pointer", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            {isAdminOnline && <span style={{ position: "absolute", top: "4px", right: "4px", width: "12px", height: "12px", backgroundColor: "#4ade80", borderRadius: "50%", border: "2px solid #3b82f6" }}></span>}
          </button>
          <ChatWidget
